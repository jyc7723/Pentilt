...
avr = serial.Serial(port='COM7', baudrate=250000)
...
def video_play():
    ...
    if ret:
        ...
        if len(keypoints) != 0:
            lz_x = keypoints[0].pt[0]
            lz_y = keypoints[0].pt[1]
...
##            print(int(px), int(py))
        ...
        if len(keypoints) != 0:
            error = math.sqrt((px - lz_x)**2+(py - lz_y)**2)
            if np.isnan(error) == False:
                error_v.set("error : " + str(int(error)))
...
error_label = tk.Label(window, textvariable = error_v, font = font)
error_label.place(x =750, y = 300)
